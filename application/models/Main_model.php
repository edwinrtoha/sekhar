<?php
    ...
    class main_model extends CI_Model{
        ...
        function admin_user_add(){
            ...
            $first_balance=$_POST['first_balance'];
            ...
            elseif($password!=$repassword){
                ...
                $this->db->query("INSERT INTO `user` (`first_name`,`last_name`,`birth_place`,`birth_date`,`address`,`email`,`username`,`password`,`role`,`first_balance`) VALUES('$first_name','$last_name','$birth_place','$birth_date','$address','$email','$username','$password','$role','$first_balance')");
                // $this->db->query("UPDATE `user` SET `first_name` = '$first_name' , `last_name` = '$last_name' , `birth_place` = '$birth_place' , `birth_date` = '$birth_date' , `address` = '$address' , `email` = '$email' , `username` = '$username' , `password` = '$password', `role` = '$role', `first_balance` = '$first_balance' WHERE `id` = '$uid'");
                $alert=array('type'=>'success','icon'=>'check','title'=>'Berhasil','dismiss'=>'yes','value'=>'Informasi akun berhasil di tambahkan');
            }
        ...
        function admin_user_edit($id){
            if($this->CI->session->userdata('id')){
                ...
                if(empty($num_rows) or $num_rows==0){
                    ...
                    elseif(empty($first_balance) or $first_balance==""){
                        $alert=array('type'=>'success','icon'=>'check','title'=>'Berhasil','dismiss'=>'yes','value'=>'Informasi akun berhasil di update [first_balance]');
                    }
                    elseif((empty($password) or $password=="") and (empty($repassword) or $password=="")){
                        $this->db->query("UPDATE `user` SET `first_name` = '$first_name' , `last_name` = '$last_name' , `birth_place` = '$birth_place' , `birth_date` = '$birth_date' , `address` = '$address' , `email` = '$email' , `username` = '$username', `role` = '$role', `first_balance` = '$first_balance' WHERE `id` = '$uid'");
        ...
        function admin_user_del(){
            if(isset($_GET['id'])){
                ...
                if($query->num_rows()==1){
                    ...
                    $alert=array('type'=>'success','icon'=>'check','title'=>'Berhasil','dismiss'=>'yes','value'=>'User berhasil di hapus');
                }
                else{
        ...
        function user_single($id){
            $query=$this->db->query("SELECT user.*,role.name AS role_name,inf_lokasi.nama AS birth_place_name FROM `user` INNER JOIN `role` ON role.id=user.role INNER JOIN `inf_lokasi` ON inf_lokasi.id=user.birth_place WHERE user.id = '$id'");
            return $query->result();
        }
        function trx_list(){
        ...
        function trx_add(){
            ...
            // $site_key = '6LdUDBYUAAAAAHYuPRCkA69-o7x7JuWC8-JVxyEi'; // Diisi dengan site_key API Google reCapthca yang sobat miliki
            // $secret_key = '6LdUDBYUAAAAAK4jeiQG2mZ_XwIuv6TOpkXZr5lQ'; // Diisi dengan secret_key API Google reCapthca yang sobat miliki
            ...
            if(isset($_GET['id'])){
                ...
                if($this->CI->session->userdata('id')){
                    ...
                    elseif(empty($date) or $date==""){
                        ...
                        $this->db->query("UPDATE `trx` SET `name` = '$name' , `type` = '$type' , `nominal` = '$nominal' , `trx_datetime` = '$date' , `description` = '$desc' WHERE `id` = '$id' AND `uid` = '$uid'");
            ...
            if($this->CI->session->userdata('id')){
                ...
                //     if($data['success']){
                        ...
                        if(empty($num_rows) or $num_rows==0){
                            ...
                            elseif((empty($password) or $password=="") and (empty($repassword) or $password=="")){
                                $this->db->query("UPDATE `user` SET `first_name` = '$first_name' , `last_name` = '$last_name' , `birth_place` = '$birth_place' , `birth_date` = '$birth_date' , `address` = '$address' , `email` = '$email' , `username` = '$username', `first_balance` = '$first_balance' WHERE `id` = '$uid'");
                                $alert=array('type'=>'success','icon'=>'check','title'=>'Berhasil','dismiss'=>'yes','value'=>'Informasi akun berhasil di update');
                            }
                            elseif($password!=$repassword){
                                $alert=array('type'=>'danger','icon'=>'ban','title'=>'Kesalahan','dismiss'=>'yes','value'=>'Form belum di isi dengan benar');
                                ...
                                $this->db->query("UPDATE `user` SET `first_name` = '$first_name' , `last_name` = '$last_name' , `birth_place` = '$birth_place' , `birth_date` = '$birth_date' , `address` = '$address' , `email` = '$email' , `username` = '$username' , `password` = '$password', `first_balance` = '$first_balance' WHERE `id` = '$uid'");
                                $alert=array('type'=>'success','icon'=>'check','title'=>'Berhasil','dismiss'=>'yes','value'=>'Informasi akun berhasil di update');
                            }
                        }
                //     }
