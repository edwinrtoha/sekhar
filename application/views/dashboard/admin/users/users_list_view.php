		...
		<div class="row">
			<div class="col-md-12">
				<?php
					...
					$data_from=1;
					$data_end=$limit;
					$query_all=$this->db->query("SELECT COUNT(*) FROM `user` ORDER BY `id` DESC");
					$users_count=$query_all->row_array();
					...
					$prev_url=base_url()."dashboard/admin/users?page=".$prev;
					$next_url=base_url()."dashboard/admin/users?page=".$next;
					...
					<div class="box-body no-padding">
						<div class="mailbox-controls">
							...
							<div class="pull-right">
								<?php echo $data_from;?>-<?php echo $data_end;?>/<?php echo $users_count;?>
								<div class="btn-group">
									<?php if($prev>0):?>
								...
								<tbody>
									...
									<?php if($query->num_rows()>0):?>
										<?php foreach ($query->result() as $row){ ?>
											<?php
												$roless=$this->db->query("SELECT * FROM `role` WHERE `id` = '$row->role'");
												foreach($roless->result() as $roles){
													$role=ucfirst($roles->name);
												}
											?>
											<tr>
												<td><input name="checkbox" type="checkbox" value="" class="flat-blue"></td>
												<td><?php echo $row->first_name." ".$row->last_name;?></td>
												<td style="max-width:650px;"><?php echo $row->username;?></td>
